(function (global) {
  ...
  // FAST V10 + NEWS V5: delta server cache + session read cache + paged/lazy endpoints.
  ...
  // FAST V10: เก็บ response หน้ารายการ/summary ใน sessionStorage
  // จึงยังเปิดได้ทันทีหลัง Refresh ใน tab เดิม โดยไม่เก็บรายละเอียด/รูป/ไฟล์ export ขนาดใหญ่
  const SESSION_READ_CACHE_KEY = 'UBU_STAFF_READ_CACHE_FAST_V10';
  ...
  function flushSessionReadCache() {
    ...
    try {
      ...
      let raw = JSON.stringify({ v: 10, entries: kept });
      while (raw.length > SESSION_READ_CACHE_MAX_BYTES && kept.length > 1) {
        kept.pop();
        raw = JSON.stringify({ v: 10, entries: kept });
      }
