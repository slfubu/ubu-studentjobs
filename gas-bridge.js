(function (global) {
  ...
  // FAST V11.2 FORM BRIDGE + NEWS V5: hidden iframe transport + preload bundle + delta cache.
  ...
  // =====================================================
  // FAST V11.2 FORM BRIDGE: native form POST + one-shot iframe
  ...
  function formBridgeUrl() {
    ...
    return base + '?transport=form_bridge&v=20260917-fast-v11-2';
  }
  ...
  function invokeAppsScriptViaFormBridge(action, args) {
    return new Promise((resolve, reject) => {
      ...
      timer = setTimeout(() => {
        ...
        finishError('Apps Script ไม่ตอบกลับผ่าน Form Bridge กรุณาตรวจสอบว่า Web App Deploy เป็นเวอร์ชันล่าสุดและอนุญาตให้เปิดใช้งานจากหน้าเว็บ');
      }, timeoutFor(action));
