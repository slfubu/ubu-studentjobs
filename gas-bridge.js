(function (global) {
  ...
  // V11.6 ANNOUNCEMENT PDF + PUBLIC CORS FIX: form bridge for ALL GitHub Pages API calls + public read cache.

  const CLIENT_ID_KEY = 'UBU_API_CLIENT_ID_V1';
  const READ_ACTIONS = new Set([
    'warmStaffLogin','getJobs','getPublicContents','getSiteVisitStats','getApplicationStatus','getApplicationProfileImage','getLegacyApplicantByIdCard',
    ...
    'getAdminDepartmentForwarding','getAdminDepartmentForwardingApplicants','getAdminDepartmentResults','getAdminDepartmentResultApplicants','getAdminDepartmentResultsExport','getAdminAnnouncementApplicants','getDepartmentApplicants',
  ...
  const READ_CACHE_TTL_MS = Object.freeze({
    ...
    // สถานะเปิด/ปิดรับเปลี่ยนได้ทันที จึงไม่ควรค้าง cache 10 นาที
    getJobs: 15000,
    getPublicContents: 300000,
    getSiteVisitStats: 120000,
    ...
    getAdminAnnouncementApplicants: 15000,
  ...
  const HEAVY_READ_ACTIONS = new Set([
    ...
    'getAdminDepartmentForwardingApplicants','getAdminDepartmentResults','getAdminDepartmentResultApplicants','getAdminDepartmentResultsExport','getAdminAnnouncementApplicants',
  ...
  const WRITE_INVALIDATES = Object.freeze({
    ...
    finalizeDepartmentSelection: ['getStaffDashboard','getDepartmentApplicants','getAdminDepartmentResults','getAdminDepartmentResultApplicants','getAdminDepartmentResultsExport','getAdminAnnouncementApplicants'],
    ...
    recordSiteVisit: ['getSiteVisitStats'],
    submitApplication: ['getApplicationStatus'],
    uploadApplicationFiles: ['getApplicationStatus'],
    generateAdminAnnouncementPdf: []
  });
  ...
  const STAFF_FORM_BRIDGE_ACTIONS = new Set([
    ...
    'getAdminDepartmentResults','getAdminDepartmentResultApplicants','getAdminDepartmentResultsExport','getAdminAnnouncementApplicants','generateAdminAnnouncementPdf',
  ...
  function formBridgeUrl() {
    ...
    return base + '?transport=form_bridge&v=20260926-announcement-v11-6';
  }
  ...
  function invokeAppsScriptViaFormBridge(action, args) {
    return new Promise((resolve, reject) => {
      ...
      timer = setTimeout(() => {
        ...
        finishError('Apps Script ไม่ตอบกลับผ่าน Form Bridge กรุณาตรวจสอบว่า Backend V11.3+ ถูก Deploy และ Web App เปิดใช้งานได้');
      }, timeoutFor(action));
  ...
  const READ_TIMEOUT_BY_ACTION = Object.freeze({
    ...
    getAdminAnnouncementApplicants: 120000,
  ...
  function timeoutFor(action) {
    ...
    if (['uploadApplicationFiles','savePublicContent','processQualificationImport','generateAdminAnnouncementPdf'].includes(name)) {
  ...
  async function invokeAppsScriptOnce(action, args) {
    ...
    try {
      let result;
      if (isGithubProduction) {
        // PUBLIC V11.4: ทุก action จาก GitHub Pages ต้องผ่าน native Form Bridge
        // ห้าม fallback ไป fetch() เพราะ Apps Script Web App ไม่มี CORS header ที่ browser ต้องการ
      ...
      else if (elapsedMs >= 4000) console.warn('[UBU API SLOW]', String(action || ''), `${elapsedMs}ms`, isGithubProduction ? '(form bridge)' : '(fetch)');
      return result;
    } catch (error) {
