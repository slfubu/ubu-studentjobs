(function (global) {
  ...
  const READ_ACTIONS = new Set(['getJobs','getPublicContents','getApplicationStatus','getLegacyApplicantByIdCard','getStaffSession','getStaffDashboard','getAdminApplicantPage','getAdminCivilRegistryIds','getStaffAccountManagement','getAdminPublicContents','getAdminQualificationApplicants','getAdminDepartmentForwarding','getAdminDepartmentResults','getDepartmentApplicants','getStaffApplicantDetail','getStaffApplicantPhotoBatch']);
  const inflightReads = new Map();
  const RETRY_ACTIONS = new Set([...READ_ACTIONS, 'staffLogin', 'staffLogout']);
  ...
  async function invokeAppsScriptOnce(action, args) {
    ...
    const defaultTimeout = Number(config.REQUEST_TIMEOUT_MS) || 60000;
    const timeoutMs = ['uploadApplicationFiles','savePublicContent','processQualificationImport'].includes(String(action||''))
      ? Math.max(defaultTimeout, 120000)
      : String(action||'') === 'submitApplication'
        ? Math.max(defaultTimeout, 90000)
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Apps Script ตอบกลับ HTTP ${response.status} กรุณาตรวจสอบ Deployment และลองใหม่`);
      }
      ...
      if (error instanceof TypeError) {
        throw new Error(
          'ไม่สามารถเชื่อมต่อ Apps Script Web App ได้ในขณะนี้ กรุณาตรวจสอบอินเทอร์เน็ต และหากยังเกิดซ้ำให้ตรวจ Deployment: doPost(e), URL /exec ปัจจุบัน และ Who has access = Anyone'
        );
      }
  ...
  async function invokeAppsScriptWithRetry(action, args) {
    ...
    for (let attempt=0; attempt<2; attempt++) {
      try { return await invokeAppsScriptOnce(action,args); }
        ...
        const retryable=/เชื่อมต่อ|ใช้เวลานาน|network|fetch|ตอบกลับมาไม่ใช่ JSON/i.test(message);
  ...
  function invokeAppsScript(action, args) {
    ...
    if (!RETRY_ACTIONS.has(method)) return invokeAppsScriptOnce(method,args);
    if (!READ_ACTIONS.has(method)) return invokeAppsScriptWithRetry(method,args);
    let key;
    try { key=method+'|'+JSON.stringify(Array.isArray(args)?args:[]); } catch (_) { key=method; }
    if (inflightReads.has(key)) return inflightReads.get(key);
    const promise=invokeAppsScriptWithRetry(method,args).finally(()=>inflightReads.delete(key));
