(function (global) {
  ...
  // FAST V11.1 CORS FIX + NEWS V5: hidden iframe transport + preload bundle + delta cache.
  ...
  // =====================================================
  // FAST V11.1 CORS FIX: hidden Apps Script iframe transport
  // =====================================================
  const BRIDGE_PAGE_URL_SUFFIX = '?page=bridge&transport=postmessage&v=20260917-fast-v11-1';
  ...
  function getBridgeUrl() {
    ...
    return base + BRIDGE_PAGE_URL_SUFFIX;
  }

  function ensureBridgeFrame() {
    if (bridgeReady && bridgeIframe && bridgeIframe.contentWindow) return Promise.resolve(bridgeIframe);
    if (bridgeReadyPromise) return bridgeReadyPromise;

    bridgeReadyPromise = new Promise((resolve, reject) => {
      bridgeReadyResolve = resolve;
      const frame = document.createElement('iframe');
      frame.id = 'ubuGasBridgeFrame';
      ...
      const timer = setTimeout(() => {
        if (!bridgeReady) {
          ...
          if (bridgeIframe === frame) bridgeIframe = null;
          reject(new Error('ไม่สามารถเริ่มการเชื่อมต่อ Apps Script Bridge ได้'));
        }
      }, 20000);
      ...
      frame.addEventListener('error', () => {
        ...
        if (!bridgeReady) {
          bridgeReadyPromise = null;
          bridgeReadyResolve = null;
          reject(new Error('ไม่สามารถโหลด Apps Script Bridge ได้'));
        }
      }, { once: true });
  ...
  global.addEventListener('message', event => {
    if (!bridgeIframe || event.source !== bridgeIframe.contentWindow) return;
    const msg = event.data || {};

    if (msg.type === 'UBU_GAS_BRIDGE_READY') {
      bridgeReady = true;
      if (bridgeReadyResolve) bridgeReadyResolve(bridgeIframe);
      bridgeReadyResolve = null;
      return;
    }

    if (msg.type !== 'UBU_GAS_BRIDGE_RESPONSE' || !msg.id) return;
    const pending = bridgePending.get(String(msg.id));
    if (!pending) return;
    bridgePending.delete(String(msg.id));
    clearTimeout(pending.timer);
    pending.resolve(msg.payload);
  });

  async function invokeAppsScriptViaBridge(action, args) {
    await ensureBridgeFrame();
    if (!bridgeIframe || !bridgeIframe.contentWindow) {
      throw new Error('Apps Script Bridge ยังไม่พร้อมใช้งาน');
    }
    ...
    const payload = await new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new Error('การเชื่อมต่อกับฐานข้อมูลไม่ตอบกลับภายในเวลาสูงสุด กรุณากดโหลดข้อมูลอีกครั้ง'));
      }, timeoutFor(action));

      bridgePending.set(id, { resolve, reject, timer });
      // The child frame validates event.origin against https://slfubu.github.io.
      // targetOrigin='*' is necessary because Apps Script HtmlService may redirect to a generated googleusercontent origin.
      bridgeIframe.contentWindow.postMessage({
        type: 'UBU_GAS_BRIDGE_REQUEST',
        id: id,
        request: request
      }, '*');
    });
  ...
  async function invokeAppsScriptOnce(action, args) {
    ...
    // Production GitHub Pages always uses the iframe bridge to avoid CORS entirely.
    // Direct fetch is kept only as a fallback for non-GitHub environments/dev testing.
    const isGithubProduction = String(global.location && global.location.origin || '') === 'https://slfubu.github.io';

    try {
      let result;
      if (isGithubProduction) {
        result = await invokeAppsScriptViaBridge(action, args);
      } else {
      ...
      else if (elapsedMs >= 4000) console.warn('[UBU API SLOW]', String(action || ''), `${elapsedMs}ms`, '(iframe bridge)');
      return result;
    } catch (error) {
