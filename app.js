const categoryConfig = {
    ...
    // FAST cache: ทำให้รายการงานแสดงได้ทันทีหลังเปิดซ้ำ/รีเฟรช
    const JOBS_LOCAL_CACHE_KEY = 'UBU_JOBS_FAST_V3_STATUS_FIX';
    // รายการงานมีการเปิด/ปิดรับได้ระหว่างวัน จึงใช้ cache สั้นเพื่อไม่ให้สถานะค้าง
    const JOBS_LOCAL_CACHE_TTL_MS = 15 * 1000;

    function readJobsLocalCache() {
      try {
        const raw = localStorage.getItem(JOBS_LOCAL_CACHE_KEY);
        if (!raw) return null;
        const parsed = JSON.parse(raw);
        if (!parsed || !Array.isArray(parsed.rows) || !parsed.rows.length) return null;
        return { rows: parsed.rows, at: Number(parsed.at) || 0 };
      } catch (_) { return null; }
    }

    function writeJobsLocalCache(rows) {
      try {
        localStorage.setItem(JOBS_LOCAL_CACHE_KEY, JSON.stringify({ at: Date.now(), rows: rows }));
      } catch (_) {}
    }
    ...
    function loadJobsFromSheet(onReady, showError = false) {
      ...
      // ใช้ local cache ก่อน เพื่อให้ UI ตอบสนองทันที
      if (!jobsLoaded && !jobsLoading) {
        const cached = readJobsLocalCache();
        if (cached && cached.rows.length) {
          jobsData = cached.rows;
          jobsLoaded = true;
          const waiters = jobsLoadWaiters.splice(0);
          waiters.forEach(fn => fn());

          // cache ยังใหม่: ไม่ต้องยิง API ซ้ำทันที
          if (Date.now() - cached.at < JOBS_LOCAL_CACHE_TTL_MS) return;
          needsServerRefresh = true;
        }
      }

      if (jobsLoaded && !jobsLoading && !needsServerRefresh) {
        const waiters = jobsLoadWaiters.splice(0);
        waiters.forEach(fn => fn());
        return;
      }
      ...
      google.script.run
        .withSuccessHandler((rows) => {
          ...
          jobsLoaded = true;
          jobsLoading = false;
          if (jobsData.length) writeJobsLocalCache(jobsData);
          const waiters = jobsLoadWaiters.splice(0);
          waiters.forEach(fn => fn());
        })
        .withFailureHandler((error) => {
          jobsLoading = false;
          const waiters = jobsLoadWaiters.splice(0);
          // ถ้ามี cache อยู่แล้ว หน้าเว็บยังใช้งานต่อได้ ไม่ล้างข้อมูล
          if (jobsData.length) {
            waiters.forEach(fn => fn());
            console.warn('ใช้รายการงานจาก cache เนื่องจาก API ตอบช้า', error);
            return;
          }
    ...
    function isJobOpen(job) {
      ...
      // ให้คอลัมน์ status เป็นค่าหลักเมื่อมีข้อมูล เช่น "เปิดรับ" / "ปิดรับ"
      const rawStatus = String(
        job.status ?? job.openStatus ?? job.applicationStatus ?? job.recruitmentStatus ?? ''
      ).trim().toLowerCase();

      if (rawStatus) {
        const closedStatuses = new Set([
          'ปิดรับ', 'ปิดรับสมัคร', 'ปิด', 'closed', 'close', 'inactive', 'false', '0', 'ไม่เปิดรับ', 'ไม่เปิดรับสมัคร'
        ]);
        const openStatuses = new Set([
          'เปิดรับ', 'เปิดรับสมัคร', 'เปิด', 'open', 'opened', 'active', 'true', '1'
        ]);
        if (closedStatuses.has(rawStatus)) return false;
        if (openStatuses.has(rawStatus)) return true;
      }

      // รองรับข้อมูลเดิมที่ backend ส่ง active มาแทน status
      if (typeof job.active === 'boolean') return job.active;
      const rawActive = String(job.active ?? '').trim().toLowerCase();
      if (['false', '0', 'no', 'off', 'ปิด', 'ปิดรับ', 'ปิดรับสมัคร'].includes(rawActive)) return false;
      if (['true', '1', 'yes', 'on', 'เปิด', 'เปิดรับ', 'เปิดรับสมัคร'].includes(rawActive)) return true;

      // ถ้าไม่มีทั้ง status และ active ให้คงพฤติกรรมเดิม คือถือว่าเปิดใช้งาน
      return true;
    }

    function normalizeJobDepartment(value) {
      return String(value || '')
        .trim()
        .toLowerCase()
        .replace(/\s+/g, '')
        .replace(/^กลุ่มงาน/, 'งาน');
    }

    function getJobsForCategory(key) {
      const config = categoryConfig[key];
      if (!config) return [];
      const expectedDepartment = normalizeJobDepartment(config.title);
      const expectedKey = normalizeJobDepartment(key);

      return jobsData
        .filter(job => {
          if (!isJobOpen(job)) return false;
          const department = normalizeJobDepartment(job.department);
          return department === expectedDepartment || department === expectedKey;
        })
        .sort((a, b) => {
    ...
    function renderPublicContentItems(rows, type) {
      ...
      return data.map(item => {
        ...
        const newsClickAttrs = type === 'news' && item.id
          ? ` onclick="openHomepageNewsItem('${escapeHtml(item.id || '')}')" role="button" tabindex="0"`
          : '';

        return `
          <article class="public-content-card ${type === 'news' ? 'public-content-news-clickable' : ''}"${newsClickAttrs}>
            <div class="public-content-date"><i class="fa-regular fa-calendar me-1"></i>วันที่ประกาศ ${escapeHtml(item.publishDate || '-')}</div>
            ${type === 'news' ? `<div class="news-view-count public-news-view-count" data-news-view-id="${escapeHtml(item.id || '')}"><i class="fa-regular fa-eye me-1"></i>เข้าอ่านแล้ว <strong>${formatNewsViewCount(item.viewCount)}</strong> คน</div>` : ''}
            <div class="public-content-subject">${escapeHtml(item.subject || '-')}</div>
            ${details ? `<div class="public-content-details">${details}</div>` : ''}
            ${type === 'news' ? `<div class="public-content-read-hint"><i class="fa-regular fa-hand-pointer me-1"></i>คลิกเพื่อเปิดอ่านประกาศ</div>` : ''}
            ${fileUrl ? `
              <div class="public-content-file">
                <a class="btn btn-outline-primary btn-sm" href="${escapeHtml(fileUrl)}" target="_blank" rel="noopener noreferrer" ${type === 'news' ? 'onclick="event.stopPropagation()"' : ''}>
    ...
    const PUBLIC_CONTENT_LOCAL_CACHE_PREFIX = 'UBU_PUBLIC_CONTENT_FAST_V3_VIEWS_';
    const PUBLIC_CONTENT_LOCAL_CACHE_TTL_MS = 2 * 60 * 1000;

    function readPublicContentLocalCache(type) {
      try {
        const raw = localStorage.getItem(PUBLIC_CONTENT_LOCAL_CACHE_PREFIX + type);
        if (!raw) return null;
        const parsed = JSON.parse(raw);
    ...
    function writePublicContentLocalCache(type, rows) {
      try {
        localStorage.setItem(PUBLIC_CONTENT_LOCAL_CACHE_PREFIX + type, JSON.stringify({ at: Date.now(), rows: rows }));
      } catch (_) {}
    }
    ...
    async function loadPublicContent(type, containerId) {
      ...
      const cached = readPublicContentLocalCache(type);
      if (cached) {
        container.innerHTML = renderPublicContentItems(cached.rows, type);
        if (Date.now() - cached.at < PUBLIC_CONTENT_LOCAL_CACHE_TTL_MS) return;
      } else {
      ...
      try {
        ...
        writePublicContentLocalCache(type, normalizedRows);
        container.innerHTML = renderPublicContentItems(normalizedRows, type);
      } catch (error) {
        if (!cached) {
          ...
          console.warn('ใช้ข่าว/ประกาศจาก cache เนื่องจาก API ตอบช้า', error);
        }
      }
    }

    let homepageNewsRows = [];
    const NEWS_SEEN_KEY = 'UBU_NEWS_SEEN_V4';
    const NEWS_DISMISSED_POPUP_KEY = 'UBU_NEWS_POPUP_DISMISSED_V4';
    const NEWS_SESSION_POPUP_KEY = 'UBU_NEWS_POPUP_SESSION_V4';

    function readNewsIdSet(key, storage = localStorage) {
      try {
    ...
    function writeNewsIdSet(key, set, storage = localStorage) {
      try { storage.setItem(key, JSON.stringify([...set].slice(-300))); } catch (_) {}
    }

    const NEWS_VIEW_COUNTED_KEY = 'UBU_NEWS_VIEW_COUNTED_V5';
    const newsViewRecordInflight = new Map();

    function formatNewsViewCount(value) {
      const count = Math.max(0, Math.floor(Number(value) || 0));
      return count.toLocaleString('th-TH');
    }

    function updateNewsViewCountEverywhere(id, value) {
      const key = String(id || '');
      const count = Math.max(0, Math.floor(Number(value) || 0));
      if (!key) return;

      homepageNewsRows.forEach(item => {
        if (item && String(item.id || '') === key) item.viewCount = count;
      });

      document.querySelectorAll('[data-news-view-id]').forEach(el => {
        if (String(el.dataset.newsViewId || '') !== key) return;
        const strong = el.querySelector('strong');
        if (strong) strong.textContent = formatNewsViewCount(count);
        else el.textContent = `เข้าอ่านแล้ว ${formatNewsViewCount(count)} คน`;
      });

      const currentModal = document.getElementById('newsPopupModal');
      if (currentModal && String(currentModal.dataset.newsId || '') === key) {
        const popupCount = document.getElementById('newsPopupViewCount');
        if (popupCount) popupCount.innerHTML = `<i class="fa-regular fa-eye me-1"></i>เข้าอ่านแล้ว <strong>${formatNewsViewCount(count)}</strong> คน`;
      }

      // อัปเดต local cache เพื่อให้กลับมาหน้าเดิมแล้วเห็นตัวเลขล่าสุดทันที
      const cached = readPublicContentLocalCache('news');
      if (cached && Array.isArray(cached.rows)) {
        cached.rows.forEach(row => {
          if (row && String(row.id || '') === key) row.viewCount = count;
        });
        writePublicContentLocalCache('news', cached.rows);
      }
    }

    async function recordNewsReadOnce(item) {
      const id = String(item && item.id || '');
      if (!id) return;

      const counted = readNewsIdSet(NEWS_VIEW_COUNTED_KEY);
      if (counted.has(id)) return;
      if (newsViewRecordInflight.has(id)) return newsViewRecordInflight.get(id);

      const request = publicServerCall('recordPublicContentView', id)
        .then(result => {
          if (result && result.success !== false) {
            if (result.counted !== false) {
              counted.add(id);
              writeNewsIdSet(NEWS_VIEW_COUNTED_KEY, counted);
            }
            updateNewsViewCountEverywhere(id, result.viewCount);
          }
          return result;
        })
        .catch(error => {
          // ตัวนับไม่ควรทำให้การอ่านข่าวล้มเหลว
          console.warn('ไม่สามารถอัปเดตจำนวนผู้อ่านข่าวได้:', error);
          return null;
        })
        .finally(() => newsViewRecordInflight.delete(id));

      newsViewRecordInflight.set(id, request);
      return request;
    }

    function markNewsSeen(ids) {
      const seen = readNewsIdSet(NEWS_SEEN_KEY);
      (Array.isArray(ids) ? ids : [ids]).filter(Boolean).forEach(id => seen.add(String(id)));
      writeNewsIdSet(NEWS_SEEN_KEY, seen);
      renderNewsUnreadBadge(homepageNewsRows);
    }

    function renderNewsUnreadBadge(rows) {
      const badge = document.getElementById('newsUnreadBadge');
      if (!badge) return;
      const seen = readNewsIdSet(NEWS_SEEN_KEY);
      const unread = (Array.isArray(rows) ? rows : []).filter(item => item && item.id && !seen.has(String(item.id))).length;
      if (!unread) {
    ...
    function shortNewsText(value, max = 150) {
      const text = String(value || '').replace(/\s+/g, ' ').trim();
      return text.length > max ? text.slice(0, max - 1).trimEnd() + '…' : text;
    }

    function renderHomepageNewsExperience(rows, options = {}) {
      homepageNewsRows = Array.isArray(rows) ? rows : [];
      renderNewsUnreadBadge(homepageNewsRows);

      const seen = readNewsIdSet(NEWS_SEEN_KEY);
      const section = document.getElementById('homepage-news-section');
      const list = document.getElementById('homepageNewsList');
      const homeRows = homepageNewsRows.filter(item => item && item.showHome !== false).slice(0, 4);
      ...
      if (section && list) {
        if (!homeRows.length) {
          ...
          list.innerHTML = homeRows.map(item => {
            const isUnread = item.id && !seen.has(String(item.id));
            return `
              <article class="homepage-news-card ${item.important ? 'is-important' : ''}" onclick="openHomepageNewsItem('${escapeHtml(item.id || '')}')" tabindex="0" role="button">
                <div class="homepage-news-meta">
                  <span><i class="fa-regular fa-calendar me-1"></i>${escapeHtml(item.publishDate || '-')}</span>
                  ${isUnread ? '<span class="homepage-news-new">NEW</span>' : ''}
                  ${item.important ? '<span class="homepage-news-important"><i class="fa-solid fa-star me-1"></i>สำคัญ</span>' : ''}
                </div>
                <div class="homepage-news-title">${escapeHtml(item.subject || '-')}</div>
                ${item.details ? `<div class="homepage-news-desc">${escapeHtml(shortNewsText(item.details, 180))}</div>` : ''}
                <div class="homepage-news-footer">
                  <div class="homepage-news-link">อ่านรายละเอียด <i class="fa-solid fa-arrow-right ms-1"></i></div>
                  <div class="news-view-count homepage-news-view-count" data-news-view-id="${escapeHtml(item.id || '')}"><i class="fa-regular fa-eye me-1"></i>เข้าอ่านแล้ว <strong>${formatNewsViewCount(item.viewCount)}</strong> คน</div>
                </div>
              </article>`;
          }).join('');
        }
      ...
      const urgent = homepageNewsRows.find(item => item && item.important);
      const urgentBar = document.getElementById('urgentAnnouncementBar');
      const urgentText = document.getElementById('urgentAnnouncementText');
      if (urgentBar && urgentText) {
        if (urgent) {
          urgentText.textContent = urgent.subject || 'มีประกาศสำคัญใหม่';
          urgentBar.dataset.newsId = String(urgent.id || '');
          urgentBar.style.display = '';
        } else {
          urgentBar.style.display = 'none';
          urgentBar.dataset.newsId = '';
        }
      }

      if (options.allowPopup) scheduleAutomaticNewsPopup(homepageNewsRows);
    }

    function getNewsById(id) {
      return homepageNewsRows.find(item => String(item && item.id || '') === String(id || '')) || null;
    }

    function openHomepageNewsItem(id, options = {}) {
      const item = getNewsById(id);
      if (!item) return;

      const title = document.getElementById('newsPopupTitle');
      const date = document.getElementById('newsPopupDate');
      const details = document.getElementById('newsPopupDetails');
      const file = document.getElementById('newsPopupFile');
      const dontShow = document.getElementById('newsPopupDontShowAgain');
      const modalEl = document.getElementById('newsPopupModal');
      if (!title || !date || !details || !file || !modalEl) return;

      title.textContent = item.subject || 'ข่าวประกาศ';
      date.innerHTML = `<div class="d-flex flex-wrap align-items-center gap-2"><span><i class="fa-regular fa-calendar me-1"></i>วันที่ประกาศ ${escapeHtml(item.publishDate || '-')}</span>${item.important ? ' <span class="homepage-news-important"><i class="fa-solid fa-star me-1"></i>ข่าวสำคัญ</span>' : ''}<span id="newsPopupViewCount" class="news-view-count news-popup-view-count" data-news-view-id="${escapeHtml(item.id || '')}"><i class="fa-regular fa-eye me-1"></i>เข้าอ่านแล้ว <strong>${formatNewsViewCount(item.viewCount)}</strong> คน</span></div>`;
      details.innerHTML = item.details ? escapeHtml(item.details).replace(/\r?\n/g, '<br>') : '<span class="text-muted">ไม่มีรายละเอียดเพิ่มเติม</span>';

      const fileUrl = safePublicFileUrl(item.fileUrl);
      file.innerHTML = fileUrl
        ? `<a href="${escapeHtml(fileUrl)}" target="_blank" rel="noopener noreferrer" class="btn btn-outline-primary"><i class="fa-solid fa-paperclip me-1"></i>เปิดไฟล์แนบ</a>`
        : '';

      if (dontShow) dontShow.checked = false;
      modalEl.dataset.newsId = String(item.id || '');
      modalEl.dataset.autoPopup = options.auto ? 'true' : 'false';
      markNewsSeen(item.id);

      const modal = bootstrap.Modal.getOrCreateInstance(modalEl);
      modal.show();

      // นับเฉพาะการที่ผู้ใช้ "คลิกเปิดอ่าน" ข่าวเอง ไม่รวม Popup ที่ระบบเปิดอัตโนมัติ
      // นับ 1 ครั้งต่อข่าวต่อ browser/device โดยประมาณ และไม่เก็บข้อมูลส่วนบุคคลของผู้อ่าน
      if (!options.auto) recordNewsReadOnce(item);
    }

    function scheduleAutomaticNewsPopup(rows) {
      const dismissed = readNewsIdSet(NEWS_DISMISSED_POPUP_KEY);
      const shownThisSession = readNewsIdSet(NEWS_SESSION_POPUP_KEY, sessionStorage);
      const item = (Array.isArray(rows) ? rows : []).find(news =>
        news && news.showPopup === true && news.id &&
        !dismissed.has(String(news.id)) && !shownThisSession.has(String(news.id))
      );
      if (!item) return;

      shownThisSession.add(String(item.id));
      writeNewsIdSet(NEWS_SESSION_POPUP_KEY, shownThisSession, sessionStorage);
      window.setTimeout(() => openHomepageNewsItem(item.id, { auto: true }), 650);
    }

    function initNewsPopupEvents() {
      const urgentBar = document.getElementById('urgentAnnouncementBar');
      if (urgentBar) {
        const openUrgent = () => urgentBar.dataset.newsId && openHomepageNewsItem(urgentBar.dataset.newsId);
        urgentBar.addEventListener('click', openUrgent);
        urgentBar.addEventListener('keydown', event => {
      ...
      const allNewsBtn = document.getElementById('newsPopupAllNewsBtn');
      if (allNewsBtn) {
      ...
      if (modalEl) {
        modalEl.addEventListener('hidden.bs.modal', () => {
          const checkbox = document.getElementById('newsPopupDontShowAgain');
          const id = String(modalEl.dataset.newsId || '');
          if (checkbox && checkbox.checked && id) {
            const dismissed = readNewsIdSet(NEWS_DISMISSED_POPUP_KEY);
            dismissed.add(id);
            writeNewsIdSet(NEWS_DISMISSED_POPUP_KEY, dismissed);
          }
        });
      }
    }
    ...
    async function initHomepageNews() {
      const cached = readPublicContentLocalCache('news');
      if (cached && Array.isArray(cached.rows)) {
        renderHomepageNewsExperience(cached.rows, { allowPopup: false });
      }

      try {
        const rows = await publicServerCall('getPublicContents', 'news');
        const normalizedRows = Array.isArray(rows) ? rows : [];
        writePublicContentLocalCache('news', normalizedRows);
        renderHomepageNewsExperience(normalizedRows, { allowPopup: true });
      } catch (error) {
        if (cached && Array.isArray(cached.rows)) {
          renderHomepageNewsExperience(cached.rows, { allowPopup: true });
        }
        console.warn('ไม่สามารถอัปเดตข่าวหน้าแรกได้:', error);
      }
    }
    ...
    async function showAnnouncementsSection() {
      ...
      const cached = readPublicContentLocalCache('news');
      if (cached && Array.isArray(cached.rows)) {
        homepageNewsRows = cached.rows;
        markNewsSeen(cached.rows.map(item => item && item.id).filter(Boolean));
      }
    }
    ...
    let statusSearchSeq = 0;

    async function searchStatus() {
      const requestSeq = ++statusSearchSeq;
      ...
      Swal.fire({
        ...
        text: 'กำลังเชื่อมต่อฐานข้อมูล กรุณารอสักครู่',
      ...
      const slowHintTimer = setTimeout(() => {
        if (requestSeq !== statusSearchSeq || !Swal.isVisible()) return;
        Swal.update({
          title: 'กำลังตรวจสอบสถานะ',
          text: 'ระบบกำลังค้นหาข้อมูลและเตรียมรูปผู้สมัคร กรุณารอสักครู่'
        });
      }, 8000);

      try {
        const result = await publicServerCall('getApplicationStatus', studentId, idCard);
        if (requestSeq !== statusSearchSeq) return;
        ...
        idCardEl.value = idCard;
        // V3: รูปอยู่ใน result แล้ว จึงแสดงพร้อมรายละเอียดทันที ไม่ยิง Drive API รอบที่ 2
        ...
        clearTimeout(slowHintTimer);
        if (requestSeq === statusSearchSeq && searchBtn) searchBtn.disabled = false;
      }
    }
    ...
    function renderApplicationDetails(result) {
      ...
      if (result.profileImageData) {
        ...
        profilePlaceholder.innerHTML = '<i class="fa-regular fa-user fa-2x mb-2"></i><br>ไม่มีรูปภาพ';
      }
    ...
    function initPublicHomepageExperience() {
    ...
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', initPublicHomepageExperience, { once: true });
    } else {
      initPublicHomepageExperience();
    }
