const categoryConfig = {
    ...
    function showStatusSection() {
      ...
      resetStatusHumanChallenge();
      loadStatusHumanChallenge(false);
    }

    function resetStatusHumanChallenge() {
      const token = document.getElementById('statusHumanToken');
      const answer = document.getElementById('statusHumanAnswer');
      const question = document.getElementById('statusHumanQuestion');
      const button = document.getElementById('statusSearchBtn');
      if (token) token.value = '';
      if (answer) { answer.value = ''; answer.disabled = true; }
      if (question) question.textContent = 'กำลังสร้างโจทย์...';
      if (button) button.disabled = true;
    }

    let statusHumanChallengeSeq = 0;
    async function loadStatusHumanChallenge(focusAnswer = false) {
      const seq = ++statusHumanChallengeSeq;
      const token = document.getElementById('statusHumanToken');
      const answer = document.getElementById('statusHumanAnswer');
      const question = document.getElementById('statusHumanQuestion');
      const hint = document.getElementById('statusHumanHint');
      const refresh = document.getElementById('statusHumanRefreshBtn');
      const searchBtn = document.getElementById('statusSearchBtn');
      if (!token || !answer || !question) return;
      ...
      question.innerHTML = '<i class="fa-solid fa-spinner fa-spin me-2"></i>กำลังสร้างโจทย์...';
      if (hint) hint.textContent = 'กำลังเชื่อมต่อระบบยืนยัน...';

      try {
        const challenge = await publicServerCall('getHumanVerificationChallenge');
        if (seq !== statusHumanChallengeSeq) return;
        if (!challenge || !challenge.token || !challenge.question) throw new Error('ไม่สามารถสร้างแบบทดสอบได้');

        token.value = String(challenge.token);
        question.textContent = String(challenge.question);
        answer.disabled = false;
        if (searchBtn) searchBtn.disabled = false;
        if (hint) hint.textContent = 'โจทย์มีอายุประมาณ 3 นาที และใช้ได้เพียงครั้งเดียว';
        if (focusAnswer) answer.focus();
      } catch (error) {
        if (seq !== statusHumanChallengeSeq) return;
        question.textContent = 'โหลดโจทย์ไม่สำเร็จ';
        if (hint) hint.textContent = error && error.message ? error.message : 'กรุณากดปุ่มเปลี่ยนโจทย์เพื่อลองใหม่';
      } finally {
        if (seq === statusHumanChallengeSeq && refresh) refresh.disabled = false;
      }
    }
    ...
    async function searchStatus() {
      ...
      const humanTokenEl = document.getElementById('statusHumanToken');
      const humanAnswerEl = document.getElementById('statusHumanAnswer');
      const searchBtn = document.getElementById('statusSearchBtn');

      const studentId = String(studentEl.value || '').replace(/\D/g, '').slice(0, 11);
      const idCard = String(idCardEl.value || '').replace(/\D/g, '').slice(0, 13);
      const humanToken = String(humanTokenEl?.value || '').trim();
      const humanAnswer = String(humanAnswerEl?.value || '')
      ...
      if (humanAnswerEl) humanAnswerEl.value = humanAnswer;
      ...
      if (!humanToken || !/^\d{1,3}$/.test(humanAnswer)) {
        Swal.fire({
          icon: 'warning',
          title: 'กรุณายืนยันความเป็นมนุษย์',
          text: 'กรุณาตอบคำถามการทดสอบความเป็นมนุษย์ก่อนตรวจสอบสถานะ',
          confirmButtonColor: '#004a99'
        });
        if (!humanToken) loadStatusHumanChallenge(true);
        else if (humanAnswerEl) humanAnswerEl.focus();
        return;
      }

      if (searchBtn) searchBtn.disabled = true;
      if (humanAnswerEl) humanAnswerEl.disabled = true;
      ...
      const slowHintTimer = setTimeout(() => {
        ...
        Swal.update({
          ...
          text: 'ระบบกำลังค้นหาข้อมูลจากฐานข้อมูล กรุณารอ ระบบจะไม่ตัดการเชื่อมต่ออัตโนมัติ'
        });
      }, 8000);
      const verySlowHintTimer = setTimeout(() => {
        if (requestSeq !== statusSearchSeq || !Swal.isVisible()) return;
        Swal.update({
          title: 'ยังคงกำลังประมวลผล',
          text: 'การเชื่อมต่อใช้เวลานานกว่าปกติ แต่ระบบยังทำงานอยู่ กรุณารอผลโดยไม่กดค้นหาซ้ำ'
        });
      }, 25000);

      try {
        const result = await publicServerCall(
          'getApplicationStatus',
          studentId,
          idCard,
          { token: humanToken, answer: humanAnswer }
        );
        ...
        // FAST: แสดงข้อมูลสถานะก่อน แล้วโหลดรูปจาก Drive แบบ background
        if (result.hasProfileImage && result.applicationId) {
          loadApplicationProfileImage(result.applicationId, studentId, idCard);
        }
      } catch (error) {
        clearTimeout(slowHintTimer);
        clearTimeout(verySlowHintTimer);
        ...
        await Swal.fire({
          icon: /มนุษย์|โจทย์|หมดอายุ|คำตอบ/.test(String(error && error.message || '')) ? 'warning' : 'error',
          title: /มนุษย์|โจทย์|หมดอายุ|คำตอบ/.test(String(error && error.message || ''))
            ? 'ยืนยันความเป็นมนุษย์ไม่สำเร็จ'
        ...
        clearTimeout(slowHintTimer);
        clearTimeout(verySlowHintTimer);
        if (requestSeq === statusSearchSeq) {
          if (searchBtn) searchBtn.disabled = false;
          if (humanAnswerEl) humanAnswerEl.disabled = false;
          // Challenge เป็น one-time token: ทุกครั้งที่ค้นหาแล้วต้องออกโจทย์ใหม่
          loadStatusHumanChallenge(false);
        }
      }
    }

    function setApplicationProfileLoading() {
      const profileImage = document.getElementById('applicationProfileImage');
      const placeholder = document.getElementById('applicationProfilePlaceholder');
      if (profileImage) {
      ...
      if (placeholder) {
        ...
        placeholder.innerHTML = '<i class="fa-solid fa-spinner fa-spin fa-2x mb-2"></i><br>กำลังโหลดรูป...';
      }
    }

    function loadApplicationProfileImage(applicationId, studentId, idCard) {
      setApplicationProfileLoading();

      google.script.run
        .withSuccessHandler((result) => {
          const profileImage = document.getElementById('applicationProfileImage');
          const placeholder = document.getElementById('applicationProfilePlaceholder');
          const data = result && result.profileImageData ? String(result.profileImageData) : '';

          if (data && profileImage && placeholder) {
            profileImage.src = data;
            profileImage.style.display = 'block';
            placeholder.style.display = 'none';
          } else if (placeholder) {
            placeholder.style.display = 'block';
            placeholder.innerHTML = '<i class="fa-regular fa-user fa-2x mb-2"></i><br>ไม่มีรูปภาพ';
          }
        })
        .withFailureHandler(() => {
          const placeholder = document.getElementById('applicationProfilePlaceholder');
          if (placeholder) {
            placeholder.style.display = 'block';
            placeholder.innerHTML = '<i class="fa-regular fa-image fa-2x mb-2"></i><br>โหลดรูปไม่สำเร็จ';
          }
        })
        .getApplicationProfileImage(applicationId, studentId, idCard);
    }
    ...
    function renderApplicationDetails(result) {
      ...
      if (result.profileImageData) {
        ...
        profilePlaceholder.innerHTML = result.hasProfileImage
          ? '<i class="fa-solid fa-spinner fa-spin fa-2x mb-2"></i><br>กำลังโหลดรูป...'
          : '<i class="fa-regular fa-user fa-2x mb-2"></i><br>ไม่มีรูปภาพ';
      }
    ...
    const statusHumanAnswerInput = document.getElementById('statusHumanAnswer');
    if (statusHumanAnswerInput) {
      statusHumanAnswerInput.addEventListener('input', function () {
        this.value = this.value
          .replace(/[๐-๙]/g, ch => String('๐๑๒๓๔๕๖๗๘๙'.indexOf(ch)))
          .replace(/\D/g, '')
          .slice(0, 3);
      });
      statusHumanAnswerInput.addEventListener('keydown', function (event) {
        if (event.key === 'Enter') searchStatus();
      });
    }
