const categoryConfig = {
    ...
    /* =========================================================
       ...
       - แสดงจำนวนเข้าชมทั้งหมด/วันนี้จาก Apps Script
       - นับ 1 ครั้งต่อเบราว์เซอร์ในช่วงเวลา 30 นาที
       - เริ่มบันทึกการเข้าชมหลังผู้ใช้ยอมรับคุกกี้
       ========================================================= */
    ...
    function renderSiteVisitStats(stats) {
      ...
      if (totalEl) totalEl.textContent = formatSiteVisitNumber(stats && stats.total);
      if (todayEl) todayEl.textContent = formatSiteVisitNumber(stats && stats.today);
    }
    ...
    async function refreshSiteVisitCounter(options = {}) {
      ...
      try {
        ...
        if (stats && stats.success !== false) {
          renderSiteVisitStats(stats);
          if (canRecord) saveLastRecordedVisitAt(now);
        }
      } catch (error) {
        // ตัวนับผู้เข้าชมเป็นข้อมูลเสริม จึงไม่รบกวนการใช้งานระบบหลักหากโหลดไม่สำเร็จ
    ...
    function initSiteVisitorCounter() {
      ...
      window.addEventListener('ubu:cookie-consent-accepted', function () {
        refreshSiteVisitCounter({ consentAccepted: true });
      });
    }
