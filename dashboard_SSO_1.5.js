      const BUILD_VERSION = '2026.09.05-SSO-V1-STAFF';
    ...
    // ========================= S-MIS SSO =========================
    const SSO_APP_ID = 'studentjobs-admin';
    const SSO_CODE_PARAM = 'sso_code';

    function getSsoCodeFromUrl_() {
      try {
        const params = new URLSearchParams(window.location.search);
        const code = String(params.get(SSO_CODE_PARAM) || '').trim();

        // S-MIS SSO Core ปัจจุบันออก token แบบ hex 128 ตัว
        // เปิดช่วงไว้เล็กน้อยเพื่อรองรับการเปลี่ยนรูปแบบ token ในอนาคต
        if (!code) return '';
        if (!/^[A-Za-z0-9_-]{40,200}$/.test(code)) return '';
        return code;
      } catch (_) {
        return '';
      }
    ...
    function removeSsoCodeFromUrl_() {
      try {
        const url = new URL(window.location.href);
        if (!url.searchParams.has(SSO_CODE_PARAM)) return;
    ...
    function persistCurrentStaffSession_() {
    ...
    async function loginWithSsoCode_(ssoCode) {
      const code = String(ssoCode || '').trim();
      if (!code) throw new Error('ไม่พบรหัสยืนยันตัวตนจาก S-MIS PORTAL');

      const bootDescription = document.querySelector('#bootView .boot-description');
      if (bootDescription) {
        bootDescription.textContent = 'กำลังยืนยันตัวตนผ่าน S-MIS PORTAL';
      }

      const result = await serverCall('staffSsoLogin', code, SSO_APP_ID);

      if (!result || !result.success) {
        throw new Error(
          result && result.message
            ? result.message
            : 'ไม่สามารถยืนยันตัวตนผ่าน S-MIS PORTAL ได้'
        );
      }
      ...
      persistCurrentStaffSession_();
      sessionStorage.removeItem('ubuStaffView');

      // sso_code เป็น One-Time Code ต้องเอาออกจาก address bar ทันทีหลังแลกสำเร็จ
      removeSsoCodeFromUrl_();

      enterApp();
    }

    async function bootstrapStaffAccess_() {
      const ssoCode = getSsoCodeFromUrl_();

      // ถ้ามี SSO Code ให้ใช้ SSO ก่อน แม้ Browser จะมี Session เก่าอยู่
      // เพื่อรองรับการสลับบัญชีจาก S-MIS PORTAL อย่างถูกต้อง
      if (ssoCode) {
        try {
          await loginWithSsoCode_(ssoCode);
          return;
        } catch (error) {
          console.warn('[S-MIS SSO]', error && error.message ? error.message : error);
          ...
          await Swal.fire({
            icon: 'warning',
            title: 'ไม่สามารถเข้าสู่ระบบผ่าน S-MIS PORTAL',
            text: error && error.message
              ? error.message
              : 'กรุณากลับไปที่ S-MIS PORTAL แล้วเปิดระบบอีกครั้ง หรือเข้าสู่ระบบด้วยรหัสผ่านเดิม',
    ...
    document.getElementById('loginForm').addEventListener('submit', async event => {
      ...
      try {
        ...
        applySession({
          ...
          authSource: result.authSource || 'password'
        });
    ...
    // เริ่มระบบ: ถ้ามี sso_code จะยืนยันตัวตนผ่าน S-MIS ก่อน
    // หากไม่มี จึงใช้ Session เดิมหรือแสดงหน้า Login แบบเดิม
    bootstrapStaffAccess_();
