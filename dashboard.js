      ...
      staffAccountRows: [],
      staffAccountUnits: [],
       applicantReviewPage: 1, applicantReviewPageSize: 20,
       civilRegistryPage: 1, civilRegistryPageSize: 20
    };
    ...
    async function renderStaffAccounts() {
      ...
      content.innerHTML = `
        <div class="staff-account-hero">
          <div>
            <div class="page-title mb-1">ลงทะเบียนผู้ใช้งานเจ้าหน้าที่</div>
            <div class="page-subtitle">กำหนดชื่อผู้ใช้ สิทธิ์ หน่วยงาน และหน่วยคัดเลือกสำหรับการเข้าใช้งานระบบ</div>
          </div>
          <div class="staff-account-count">
            <i class="fa-solid fa-users-gear"></i>
            <span>บัญชีทั้งหมด <strong>${state.staffAccountRows.length.toLocaleString('th-TH')}</strong></span>
          </div>
        </div>
        ...
        <div class="row g-3 align-items-stretch">
          <div class="col-12 col-xl-5">
            <div class="panel h-100">
              <div class="panel-header">
                <div>
                  <h2 class="panel-title"><i class="fa-solid fa-user-plus me-2"></i>สร้างบัญชีใหม่</h2>
                  <div class="text-muted small mt-1">รหัสผ่านจะถูกจัดเก็บแบบเข้ารหัสแฮช ไม่แสดงกลับในตาราง</div>
                </div>
              </div>
                  ...
                  <div class="row g-3">
                    <div class="col-12">
                      ...
                      <input id="staffAccountUsername" class="form-control" maxlength="80" autocomplete="off" required
                             placeholder="เช่น welfare01">
                      <div class="form-text">ใช้ตัวอักษร ตัวเลข จุด ขีดกลาง หรือขีดล่าง และห้ามซ้ำกับบัญชีเดิม</div>
                    </div>

                    <div class="col-12">
                      <label class="form-label">ชื่อที่แสดง <span class="text-danger">*</span></label>
                      <input id="staffAccountDisplayName" class="form-control" maxlength="150" required
                    ...
                    <div class="col-12 col-md-6">
                      ...
                      <div class="input-group">
                        <input id="staffAccountPassword" type="password" class="form-control" minlength="8" maxlength="200"
                               autocomplete="new-password" required placeholder="อย่างน้อย 8 ตัวอักษร">
                        <button class="btn btn-outline-secondary" type="button" onclick="toggleStaffPassword('staffAccountPassword', this)" aria-label="แสดงหรือซ่อนรหัสผ่าน">
                    ...
                    <div class="col-12 col-md-6">
                      ...
                      <div class="input-group">
                        <input id="staffAccountPasswordConfirm" type="password" class="form-control" minlength="8" maxlength="200"
                               autocomplete="new-password" required placeholder="กรอกซ้ำอีกครั้ง">
                        <button class="btn btn-outline-secondary" type="button" onclick="toggleStaffPassword('staffAccountPasswordConfirm', this)" aria-label="แสดงหรือซ่อนรหัสผ่าน">
                          <i class="fa-solid fa-eye"></i>
                        </button>
                      </div>
                    </div>
                    ...
                    <div class="col-12 pt-1">
                      <button id="staffAccountSubmitBtn" class="btn btn-primary w-100 py-2" type="submit">
                        <i class="fa-solid fa-user-plus me-2"></i>ลงทะเบียนผู้ใช้งานเจ้าหน้าที่
                      </button>
                    </div>
                  </div>
                </form>
          ...
          <div class="col-12 col-xl-7">
            <div class="panel h-100">
              <div class="panel-header">
                <div>
                  <h2 class="panel-title">บัญชีเจ้าหน้าที่ในระบบ</h2>
                  <div class="text-muted small mt-1">แสดงเฉพาะข้อมูลบัญชีที่จำเป็น โดยไม่เปิดเผยรหัสผ่าน</div>
                </div>
              ...
              <div class="table-wrap">
                <table class="table align-middle staff-account-table">
                  ...
                  <tbody>
                    ${renderStaffAccountRows(state.staffAccountRows)}
                  </tbody>
                </table>
              </div>
            </div>
    ...
    function renderStaffAccountRows(rows) {
      if (!Array.isArray(rows) || !rows.length) {
        return `<tr><td colspan="5" class="text-center text-muted py-5">ยังไม่มีบัญชีเจ้าหน้าที่ในระบบ</td></tr>`;
      }
      ...
      return rows.map((item, index) => {
        ...
        return `
          <tr>
            ...
            <td><span class="candidate-code fw-semibold">${escapeHtml(item.username || '-')}</span></td>
            <td>${escapeHtml(item.displayName || '-')}</td>
            <td>${roleBadge}</td>
            <td>
              ${role === 'admin'
                ? `<span class="text-muted">สิทธิ์ส่วนกลาง</span>`
                : `<div class="staff-account-unit"><strong>${escapeHtml(department)}</strong><span>${escapeHtml(unit)}</span></div>`}
            </td>
          </tr>`;
      }).join('');
    }
    ...
    function toggleStaffPassword(inputId, button) {
    ...
    async function submitStaffAccountForm(event) {
      ...
      const password = document.getElementById('staffAccountPassword').value;
      const confirmPassword = document.getElementById('staffAccountPasswordConfirm').value;
      const button = document.getElementById('staffAccountSubmitBtn');

      if (!/^[A-Za-z0-9._-]{3,80}$/.test(username)) {
        Swal.fire({ icon: 'warning', title: 'ชื่อผู้ใช้ไม่ถูกต้อง', text: 'ชื่อผู้ใช้ต้องมี 3–80 ตัวอักษร และใช้ได้เฉพาะ A-Z, a-z, 0-9, จุด, ขีดกลาง และขีดล่าง' });
        return;
      }
      if (!displayName) {
        Swal.fire({ icon: 'warning', title: 'กรอกข้อมูลไม่ครบ', text: 'กรุณาระบุชื่อที่แสดง' });
        return;
      }
      ...
      if (password.length < 8) {
        Swal.fire({ icon: 'warning', title: 'รหัสผ่านสั้นเกินไป', text: 'กรุณากำหนดรหัสผ่านอย่างน้อย 8 ตัวอักษร' });
        return;
      }
      if (password !== confirmPassword) {
        Swal.fire({ icon: 'warning', title: 'รหัสผ่านไม่ตรงกัน', text: 'กรุณากรอกรหัสผ่านและยืนยันรหัสผ่านให้ตรงกัน' });
        return;
      }
      ...
      const confirm = await Swal.fire({
        ...
        html: `ต้องการสร้างบัญชี <strong>${escapeHtml(username)}</strong> ใช่หรือไม่`,
        showCancelButton: true,
      ...
      const oldHtml = button.innerHTML;
      button.disabled = true;
      button.innerHTML = `<i class="fa-solid fa-spinner fa-spin me-2"></i>กำลังบันทึก`;
      ...
      try {
        ...
        if (!result || !result.success) {
          throw new Error(result && result.message ? result.message : 'ไม่สามารถสร้างบัญชีได้');
        }

        await Swal.fire({
          icon: 'success',
          title: 'ลงทะเบียนสำเร็จ',
          text: result.message || 'สร้างบัญชีเจ้าหน้าที่เรียบร้อยแล้ว',
          confirmButtonText: 'ตกลง'
        });
        await renderStaffAccounts();
      } catch (error) {
        Swal.fire({ icon: 'error', title: 'ลงทะเบียนไม่สำเร็จ', text: error.message });
      } finally {
    ...
    async function renderApplicantReview() {
      ...
      const statusOptions = [...new Set(state.adminApplicantRows.map(item => item.status).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'th'));
      const departmentOptions = [...new Set(state.adminApplicantRows.map(item => item.selectionUnit || item.department).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'th'));
      const facultyOptions = [...new Set(state.adminApplicantRows.map(item => item.faculty).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'th'));

      content.innerHTML = `
        <div class="d-flex flex-wrap justify-content-between gap-3 align-items-end mb-4">
          <div><div class="page-title">ตรวจสอบข้อมูลผู้สมัคร</div><div class="page-subtitle">ตรวจสอบข้อมูลที่นักศึกษากรอกในใบสมัคร รูปถ่าย และไฟล์แนบก่อนดำเนินการขั้นตอนถัดไป</div></div>
        </div>
        <div class="row g-3 mb-4">
          ${watermarkStatCard('fa-users', total, 'ผู้สมัครทั้งหมด', 'users')}
          ${watermarkStatCard('fa-clipboard-check', reviewed, 'ตรวจคุณสมบัติแล้ว', 'check')}
          ${watermarkStatCard('fa-hourglass-half', pendingReview, 'ยังไม่ตรวจสอบ', 'clock')}
          ${watermarkStatCard('fa-paper-plane', forwarded, 'ส่งให้หน่วยงานแล้ว', 'plane')}
        </div>
        <div class="panel">
          <div class="panel-header align-items-end">
            <div><h2 class="panel-title">รายชื่อผู้สมัครทั้งหมด</h2><div class="text-muted mt-1" style="font-size:13px;">ระบบแสดงเฉพาะหน้าที่เลือกเพื่อลดภาระการเรนเดอร์</div></div>
            <div class="d-flex flex-wrap gap-2 align-items-center">
              <select id="applicantReviewDepartmentFilter" class="form-select form-select-sm" style="width:220px" onchange="filterApplicantReviewTable()"><option value="">ทุกหน่วยงาน</option>${departmentOptions.map(v=>`<option>${escapeHtml(v)}</option>`).join('')}</select>
              <select id="applicantReviewFacultyFilter" class="form-select form-select-sm" style="width:220px" onchange="filterApplicantReviewTable()"><option value="">ทุกคณะ</option>${facultyOptions.map(v=>`<option>${escapeHtml(v)}</option>`).join('')}</select>
              <select id="applicantReviewStatusFilter" class="form-select form-select-sm" style="width:170px" onchange="filterApplicantReviewTable()"><option value="">ทุกสถานะ</option>${statusOptions.map(v=>`<option>${escapeHtml(v)}</option>`).join('')}</select>
              <select id="applicantReviewPageSize" class="form-select form-select-sm" style="width:110px" onchange="changeApplicantReviewPageSize(this.value)"><option>20</option><option>50</option><option>100</option></select>
              <input id="applicantReviewSearch" class="form-control form-control-sm" style="width:280px;max-width:100%" placeholder="ค้นหารหัส / ชื่อ / คณะ / งาน" oninput="filterApplicantReviewTable()">
            </div>
          </div>
          <div class="table-wrap"><table class="table table-hover"><thead><tr><th>ลำดับ</th><th>วันที่สมัคร</th><th>รหัสนักศึกษา</th><th>ชื่อ-สกุล</th><th>คณะ / สาขา</th><th>ชั้นปี</th><th>GPAX</th><th>งานที่สมัคร</th><th>สถานะ</th><th class="text-center">จัดการ</th></tr></thead><tbody id="applicantReviewBody"></tbody></table></div>
          <div id="applicantReviewPager" class="p-3 border-top"></div>
        </div>`;
      refreshApplicantReviewPage();
    }

    function watermarkStatCard(icon, number, label, mark='dot') {
      return `<div class="col-6 col-xl-3"><div class="stat-card stat-card-watermark"><div class="stat-watermark"><i class="fa-solid ${icon}"></i></div><div class="position-relative"><div class="stat-icon mb-3"><i class="fa-solid ${icon}"></i></div><div class="stat-number">${Number(number||0).toLocaleString('th-TH')}</div><div class="stat-label">${escapeHtml(label)}</div></div></div></div>`;
    }

    function getApplicantReviewFilteredRows() {
      ...
      const department=document.getElementById('applicantReviewDepartmentFilter')?.value||'';
      const faculty=document.getElementById('applicantReviewFacultyFilter')?.value||'';
      return state.adminApplicantRows.filter(item => {
        const hay=`${item.studentId} ${item.fullName} ${item.faculty} ${item.major} ${item.job} ${item.department} ${item.selectionUnit}`.toLowerCase();
        return (!search||hay.includes(search)) && (!status||item.status===status) && (!department||(item.selectionUnit||item.department)===department) && (!faculty||item.faculty===faculty);
      });
    }

    function refreshApplicantReviewPage() {
      const rows=getApplicantReviewFilteredRows();
      const size=Number(state.applicantReviewPageSize)||20;
      const pages=Math.max(1,Math.ceil(rows.length/size));
      state.applicantReviewPage=Math.min(Math.max(1,state.applicantReviewPage),pages);
      const start=(state.applicantReviewPage-1)*size;
      renderApplicantReviewRows(rows.slice(start,start+size), start);
      renderSimplePager('applicantReviewPager', rows.length, state.applicantReviewPage, size, 'goApplicantReviewPage');
    }
    function filterApplicantReviewTable(){ state.applicantReviewPage=1; refreshApplicantReviewPage(); }
    function changeApplicantReviewPageSize(v){ state.applicantReviewPageSize=[20,50,100].includes(Number(v))?Number(v):20; state.applicantReviewPage=1; refreshApplicantReviewPage(); }
    function goApplicantReviewPage(p){ state.applicantReviewPage=Number(p)||1; refreshApplicantReviewPage(); }

    function renderApplicantReviewRows(rows, offset=0) {
      const body=document.getElementById('applicantReviewBody'); if(!body) return;
      body.innerHTML=rows.length ? rows.map((item,index)=>`<tr><td>${offset+index+1}</td><td class="text-nowrap">${escapeHtml(item.submittedAt||'-')}</td><td class="candidate-code fw-medium">${escapeHtml(item.studentId||'-')}</td><td class="candidate-name">${escapeHtml(item.fullName||'-')}</td><td><div>${escapeHtml(item.faculty||'-')}</div>${item.major?`<div class="text-muted" style="font-size:12px">${escapeHtml(item.major)}</div>`:''}</td><td>${escapeHtml(item.year||'-')}</td><td class="fw-semibold">${escapeHtml(item.gpax||'-')}</td><td style="min-width:220px"><div>${escapeHtml(item.job||'-')}</div>${item.department?`<div class="text-muted" style="font-size:12px">${escapeHtml(item.selectionUnit||item.department)}</div>`:''}</td><td>${statusBadge(item.status)}</td><td class="text-center text-nowrap"><button class="btn btn-outline-primary btn-sm" onclick="showApplicantDetail('${escapeHtml(item.applicationId)}')"><i class="fa-solid fa-magnifying-glass me-1"></i>ตรวจสอบข้อมูล</button></td></tr>`).join('') : `<tr><td colspan="10" class="text-center text-muted py-4">ไม่พบข้อมูลตามตัวกรอง</td></tr>`;
    }
    ...
    function renderSimplePager(targetId,total,page,size,handler){
      ...
      el.innerHTML=`<div class="d-flex flex-wrap justify-content-between align-items-center gap-2"><div class="text-muted small">แสดง ${total?((page-1)*size+1):0}-${Math.min(page*size,total)} จาก ${total.toLocaleString('th-TH')} รายการ</div><div class="btn-group btn-group-sm"><button class="btn btn-outline-secondary" ${page<=1?'disabled':''} onclick="${handler}(${page-1})"><i class="fa-solid fa-chevron-left"></i></button><button class="btn btn-light disabled">หน้า ${page}/${pages}</button><button class="btn btn-outline-secondary" ${page>=pages?'disabled':''} onclick="${handler}(${page+1})"><i class="fa-solid fa-chevron-right"></i></button></div></div>`;
    }
    ...
    async function renderCivilRegistry() {
      ...
      content.innerHTML = `
        ...
        <div class="row g-3 mb-4">
          ${watermarkStatCard('fa-users', total, 'ผู้สมัครทั้งหมด')}
          ${watermarkStatCard('fa-user-check', verified, 'ตรวจสอบแล้ว')}
          ${watermarkStatCard('fa-id-card-clip', invalid, 'เลข 13 หลักไม่ถูกต้อง')}
          ${watermarkStatCard('fa-clock', pending, 'ยังไม่ตรวจสอบ')}
        </div>
        ...
        <div class="panel">
          <div class="panel-header">
            ...
            <div class="d-flex flex-wrap gap-2 align-items-center">
              ...
              <select id="civilRegistryPageSize" class="form-select form-select-sm" style="width:110px" onchange="changeCivilRegistryPageSize(this.value)"><option>20</option><option>50</option><option>100</option></select><input id="civilRegistrySearch" class="form-control form-control-sm" style="width:300px; max-width:100%;" placeholder="ค้นหารหัสนักศึกษา / ชื่อ / คณะ" oninput="filterCivilRegistryTable()">
            </div>
          </div>
          <div class="table-wrap">
          ...
          <div id="civilRegistryPager" class="p-3 border-top"></div>
        </div>
      `;
      refreshCivilRegistryPage();
    }

    function getCivilRegistryFilteredRows() {
      ...
      return state.civilRegistryRows.filter(item=>{
        const filterState=item.checksumStatus==='ไม่ถูกต้อง'?'invalid':item.verificationStatus==='ตรวจสอบเลขประจำตัวแล้ว'?'verified':'pending';
        const hay=`${item.studentId} ${item.fullName} ${item.faculty}`.toLowerCase();
        return (!search||hay.includes(search)) && (!filter||filterState===filter);
      });
    }
    function refreshCivilRegistryPage(){
      const rows=getCivilRegistryFilteredRows(); const size=Number(state.civilRegistryPageSize)||20; const pages=Math.max(1,Math.ceil(rows.length/size));
      state.civilRegistryPage=Math.min(Math.max(1,state.civilRegistryPage),pages); const start=(state.civilRegistryPage-1)*size;
      renderCivilRegistryRows(rows.slice(start,start+size),start); renderSimplePager('civilRegistryPager',rows.length,state.civilRegistryPage,size,'goCivilRegistryPage');
      const box=document.getElementById('civilRegistrySelectAll'); if(box) box.checked=false;
    }
    function changeCivilRegistryPageSize(v){ state.civilRegistryPageSize=[20,50,100].includes(Number(v))?Number(v):20; state.civilRegistryPage=1; refreshCivilRegistryPage(); }
    function goCivilRegistryPage(p){ state.civilRegistryPage=Number(p)||1; refreshCivilRegistryPage(); }
    function renderCivilRegistryRows(rows, offset=0) {
      const body=document.getElementById('civilRegistryBody'); if(!body) return;
      body.innerHTML=rows.length ? rows.map((item,index)=>`<tr><td class="text-center"><input class="form-check-input civil-row-check" type="checkbox" value="${escapeHtml(item.applicationId)}"></td><td>${offset+index+1}</td><td class="candidate-code fw-medium">${escapeHtml(item.studentId||'-')}</td><td><div class="candidate-name">${escapeHtml(item.fullName||'-')}</div><div class="text-muted" style="font-size:12px">${escapeHtml(item.faculty||'-')}</div></td><td class="text-nowrap">${escapeHtml(item.idCardMasked||'-')}</td><td>${civilChecksumBadge(item.checksumStatus)}</td><td>${civilRegistryValueBadge(item.personStatus)}</td><td>${civilRegistryValueBadge(item.domicileStatus)}</td><td>${civilRegistryValueBadge(item.residenceStatus)}</td><td>${civilRegistryValueBadge(item.nationality)}</td><td>${civilVerificationBadge(item.verificationStatus)}${item.verifiedAt?`<div class="text-muted mt-1" style="font-size:11px">${escapeHtml(item.verifiedAt)}</div>`:''}</td><td class="text-center text-nowrap"><button class="btn btn-outline-primary btn-sm" onclick="runCivilRegistrySingle('${escapeHtml(item.applicationId)}')"><i class="fa-solid fa-shield-halved me-1"></i>ตรวจสอบ</button></td></tr>`).join('') : `<tr><td colspan="12" class="text-center text-muted py-4">ไม่พบข้อมูลตามตัวกรอง</td></tr>`;
    }
    function maskThaiIdClient(value){ const id=String(value||'').replace(/\D/g,''); if(id.length!==13)return id||'-'; return `${id.slice(0,1)}-XXXX-XXXXX-${id.slice(-3)}`; }
    ...
    function filterCivilRegistryTable(){ state.civilRegistryPage=1; refreshCivilRegistryPage(); }
                ...
                ${groups.length ? groups.map((group, index) => {
                  ...
                  return `
                    <tr>
                      ...
                      <td class="text-center"><button class="btn btn-link btn-sm text-decoration-none fw-semibold" onclick="showForwardingAllApplicants(${index})">${group.total} คน</button></td>
                      <td class="text-center">
                      ...
                      <td class="text-end">
                        <button class="btn btn-sm btn-outline-success me-1" onclick="downloadForwardingGroup(${index})"><i class="fa-solid fa-file-excel me-1"></i>ดาวน์โหลด</button><button class="btn btn-sm ${group.unforwardedPass > 0 ? 'btn-primary' : 'btn-outline-secondary'}" ${group.unforwardedPass > 0 ? '' : 'disabled'} onclick="sendToDepartment(${index})">
    ...
    function downloadForwardingGroup(index){
      ...
      if(window.XLSX){ const ws=XLSX.utils.aoa_to_sheet(aoa); const wb=XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb,ws,'รายชื่อ'); XLSX.writeFile(wb,`รายชื่อ_${safe}.xlsx`); } else downloadCsv(`รายชื่อ_${safe}.csv`,aoa);
    }
    ...
    async function restoreSession() {
      ...
      try {
        ...
        sessionStorage.removeItem('ubuStaffView');
        applySession({});
        /* แสดงหน้า Login เจ้าหน้าที่ใหม่โดยตรง */
        showLoginView();
      }
    }
