      const BUILD_VERSION = '2026.09.02-R3';
      ...
      staffAccountUnits: [],
       ...
       departmentPage: 1, departmentPageSize: 20
    };
    ...
    async function renderAdminDashboard() {
      ...
      content.innerHTML = `
        <section class="dashboard-hero mb-4">
          <div>
            <div class="dashboard-hero-kicker"><i class="fa-solid fa-shield-halved me-2"></i>ADMINISTRATION CENTER · ${BUILD_VERSION}</div>
            <h1>Dashboard ผู้ดูแลระบบ</h1>
            <p>ติดตามจำนวนผู้สมัคร การตรวจสอบคุณสมบัติ และการส่งรายชื่อให้หน่วยงานแบบสรุปในหน้าเดียว</p>
          </div>
          <button class="dashboard-refresh" onclick="renderAdminDashboard()"><i class="fa-solid fa-rotate me-2"></i>อัปเดตข้อมูล</button>
        </section>
    ...
    async function renderApplicantReview() {
      ...
      const content = document.getElementById('content');
      content.innerHTML = `
        <section class="section-hero applicant-hero mb-4">
          <div><div class="section-kicker">APPLICANT REVIEW</div><h1>ตรวจสอบข้อมูลผู้สมัคร</h1><p>กรองตามหน่วยงาน คณะ และสถานะได้ทันที โดยระบบดึงข้อมูลเฉพาะ 20/50/100 รายการของหน้าปัจจุบัน</p></div>
          <div class="section-hero-icon"><i class="fa-solid fa-address-card"></i></div>
        </section>
    ...
    function getApplicantReviewOptions() {
      return {
        mode: 'review', page: state.applicantReviewPage, pageSize: state.applicantReviewPageSize,
    ...
    async function loadApplicantReviewPage(firstLoad=false) {
      ...
      try {
        const result = await serverCall('getAdminApplicantPage', state.token, getApplicantReviewOptions());
    ...
    async function renderCivilRegistry() {
      ...
      const content = document.getElementById('content');
      content.innerHTML = `
        <section class="section-hero civil-hero mb-4">
          <div><div class="section-kicker">IDENTITY VERIFICATION</div><h1>ตรวจสอบข้อมูลบุคคลกับฐานข้อมูลทะเบียนราษฎร</h1><p>กรมการปกครอง · ตรวจความถูกต้องของเลขประจำตัวประชาชนและสถานะข้อมูลที่เกี่ยวข้อง</p></div>
          <div class="section-hero-icon"><i class="fa-solid fa-id-card-clip"></i></div>
        </section>
        <div class="d-flex flex-wrap justify-content-end gap-2 mb-3"><button class="btn btn-outline-primary" onclick="runCivilRegistryCheck('selected')"><i class="fa-solid fa-list-check me-1"></i>ตรวจสอบที่เลือก</button><button class="btn btn-primary" onclick="runCivilRegistryCheck('all')"><i class="fa-solid fa-users-gear me-1"></i>ตรวจสอบทั้งหมดตามตัวกรอง</button></div>
    ...
    function getCivilRegistryOptions(){ return { mode:'civil', page:state.civilRegistryPage, pageSize:state.civilRegistryPageSize, search:(document.getElementById('civilRegistrySearch')?.value||'').trim(), department:document.getElementById('civilRegistryDepartmentFilter')?.value||'', faculty:document.getElementById('civilRegistryFacultyFilter')?.value||'', registryStatus:document.getElementById('civilRegistryFilter')?.value||'' }; }
    ...
    async function loadCivilRegistryPage(firstLoad=false){
      ...
      const result=await serverCall('getAdminApplicantPage',state.token,getCivilRegistryOptions());
    ...
    async function renderForwarding() {
      ...
      content.innerHTML=`
        <section class="section-hero forwarding-hero mb-4"><div><div class="section-kicker">FORWARD TO DEPARTMENTS</div><h1>ส่งรายชื่อให้หน่วยงานคัดเลือก</h1><p>ตรวจสอบจำนวนผู้สมัคร เปิดดูรายชื่อ และดาวน์โหลดไฟล์แยกตามหน่วยงานได้จากตารางนี้</p></div><div class="section-hero-icon"><i class="fa-solid fa-paper-plane"></i></div></section>
    ...
    async function renderDepartmentSelection() {
      ...
      content.innerHTML=`
        <section class="section-hero department-hero mb-4"><div><div class="section-kicker">DEPARTMENT SELECTION</div><h1>คัดเลือกรายชื่อลงตะกร้า</h1><p>แสดงเฉพาะรายชื่อที่ส่งมายัง <strong>${escapeHtml(state.selectionUnit||state.department)}</strong></p></div><button class="basket-widget" onclick="openBasketPage()"><span class="basket-icon-wrap"><i class="fa-solid fa-basket-shopping"></i><span id="basketCountBadge" class="basket-badge">${basketCount}</span></span><span><span class="basket-widget-label d-block">ตะกร้ารายชื่อ</span><span id="basketCountText" class="basket-widget-count d-block">${basketCount} รายชื่อ</span></span><i class="fa-solid fa-chevron-right ms-auto"></i></button></section>
    ...
    async function restoreSession() {
      ...
      try {
        ...
        const invalidSession = /เซสชัน|กรุณาเข้าสู่ระบบ|ไม่มีสิทธิ์/i.test(message);
        if (invalidSession) {
          sessionStorage.removeItem('ubuStaffSession');
          sessionStorage.removeItem('ubuStaffView');
          applySession({});
        ...
        // หากเป็น network/CORS ชั่วคราว อย่าทิ้ง session ที่ยังอาจใช้ได้
        showLoginView();
        Swal.fire({
          icon: 'warning',
          title: 'เชื่อมต่อระบบไม่สำเร็จ',
          text: message || 'ไม่สามารถตรวจสอบสถานะการเข้าสู่ระบบได้ กรุณาลองใหม่',
          confirmButtonText: 'ตกลง'
        });
      }
    }
