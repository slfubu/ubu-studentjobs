      const BUILD_VERSION = '2026.09.26-V11.6-ANNOUNCEMENT-PDF';
    ...
    async function navigate(view) {
      ...
      try {
        ...
        if (view === 'announcementPdf') return await renderAnnouncementPdf();
    ...
    function downloadBase64Blob(base64, mimeType, filename) {
      ...
      if (!clean) throw new Error('ระบบไม่ได้ส่งข้อมูลไฟล์ PDF กลับมา');
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = filename || 'ประกาศรายชื่อผู้ผ่านการคัดเลือก.pdf';
      document.body.appendChild(link);
      link.click();
      link.remove();
    ...
    async function generateAnnouncementPdf() {
      const rows = sortAnnouncementApplicants(window.announcementApplicants || []);
      if (!rows.length) {
        Swal.fire({ icon: 'info', title: 'ยังไม่มีรายชื่อ', text: 'ยังไม่มีผู้ผ่านการคัดเลือกสำหรับจัดทำประกาศ' });
        return;
      }

      const title = (document.getElementById('announcementTitle')?.value || '').trim() || 'รายชื่อผู้ผ่านการคัดเลือก';
      const subtitle = (document.getElementById('announcementSubtitle')?.value || '').trim();
      const dateText = (document.getElementById('announcementDate')?.value || '').trim() || thaiDateInputDefault();

      showLoading('กำลังจัดทำไฟล์ PDF');
      try {
        const file = await serverCall('generateAdminAnnouncementPdf', state.token, { title, subtitle, dateText });
        closeLoading();
        if (!file || !file.base64) throw new Error('ไม่สามารถสร้างไฟล์ PDF ได้');
        downloadBase64Blob(file.base64, file.mimeType || 'application/pdf', file.fileName || 'ประกาศรายชื่อผู้ผ่านการคัดเลือก.pdf');
        Swal.fire({ icon: 'success', title: 'จัดทำ PDF สำเร็จ', text: `จัดทำรายชื่อจำนวน ${Number(file.count || rows.length).toLocaleString('th-TH')} คนแล้ว`, timer: 1800, showConfirmButton: false });
      } catch (error) {
        closeLoading();
        Swal.fire({ icon: 'error', title: 'จัดทำ PDF ไม่สำเร็จ', text: error.message });
      }
    }

    async function renderAnnouncementPdf() {
      setHeader('จัดทำประกาศรายชื่อ', 'สร้างไฟล์ PDF รายชื่อผู้ผ่านการคัดเลือก');
      const content = document.getElementById('content');
      ...
      content.innerHTML = `
        <div class="announcement-hero">
          <div>
            ...
            <div class="page-subtitle">เรียงตามรหัสคณะจากรหัสนักศึกษาตัวที่ 3-4 จากน้อยไปมาก และภายในคณะเรียงตามปีเข้าศึกษาจากตัวที่ 1-2 เช่น 65 → 66 → 67</div>
          </div>
          <button class="btn btn-danger" onclick="generateAnnouncementPdf()" ${rows.length ? '' : 'disabled'}>
            <i class="fa-solid fa-file-pdf me-2"></i>จัดทำไฟล์ PDF
          </button>
        </div>
        ...
        <div class="panel announcement-settings-panel mb-4">
          <div class="panel-header">
            <div>
              <h2 class="panel-title">ข้อมูลหัวประกาศ</h2>
              <div class="small text-muted mt-1">หัวข้อและวันที่ส่วนนี้จะถูกนำไปแสดงในเอกสาร PDF</div>
            </div>
          </div>
          <div class="announcement-form-grid">
            <div>
              <label class="form-label">หัวข้อเอกสาร</label>
              <input id="announcementTitle" class="form-control" value="รายชื่อผู้ผ่านการคัดเลือก" maxlength="150">
            </div>
            <div>
              <label class="form-label">วันที่ประกาศ</label>
              <input id="announcementDate" class="form-control" value="${escapeHtml(thaiDateInputDefault())}" maxlength="100">
            </div>
            <div class="announcement-form-wide">
              <label class="form-label">ข้อความใต้หัวข้อ</label>
              <input id="announcementSubtitle" class="form-control" value="โครงการจ้างงานนักศึกษาระหว่างเรียน ประจำปีงบประมาณ 2570" maxlength="220">
            </div>
          </div>
        </div>
        ...
        <div class="panel announcement-preview-panel">
          <div class="panel-header announcement-preview-header">
            <div>
              ...
              <div class="small text-muted mt-1">ตาราง PDF: ลำดับที่ · รหัสนักศึกษา · ชื่อ-สกุล · คณะ</div>
            </div>
