      const BUILD_VERSION = '2026.09.05-SSO-CENTRAL-AUTH-V2';
    ...
    async function renderStaffAccounts() {
      setHeader('กำหนดสิทธิ์ผู้ใช้งาน', 'อนุญาต Username จาก S-MIS PORTAL และกำหนดระดับสิทธิ์/หน่วยงาน');
      const content = document.getElementById('content');
      content.innerHTML = `<div class="text-center py-5"><div class="spinner-border text-primary"></div></div>`;
      ...
      content.innerHTML = `
        <div class="d-flex flex-wrap justify-content-between gap-3 align-items-end mb-4">
          <div><div class="page-title">กำหนดสิทธิ์ผู้ใช้งาน</div><div class="page-subtitle">Username และ Password จัดการที่ S-MIS PORTAL ส่วนหน้านี้กำหนดเฉพาะสิทธิ์ของระบบ 1.5</div></div>
                  ...
                  <div class="row g-3">
                    ...
                    <div class="col-12 pt-1">
                      <button id="staffAccountSubmitBtn" class="btn btn-primary w-100 py-2" type="submit">
                        <i class="fa-solid fa-user-shield me-2"></i>อนุญาตผู้ใช้งาน
                      </button>
                    </div>
                  </div>
                </form>
            ...
            <div class="panel h-100">
              <div class="panel-header">
                <div>
                  ...
                  <div class="text-muted small mt-1">ระบบ 1.5 ไม่จัดเก็บรหัสผ่าน ใช้ข้อมูลยืนยันตัวตนจาก S-MIS PORTAL</div>
                </div>
    ...
    async function submitStaffAccountForm(event) {
      ...
      const confirm = await Swal.fire({
        icon: 'question',
        title: 'ยืนยันการอนุญาตผู้ใช้งาน',
        html: `ต้องการอนุญาต <strong>${escapeHtml(username)}</strong> ให้เข้าใช้งานระบบ 1.5 ใช่หรือไม่`,
        showCancelButton: true,
        confirmButtonText: 'ยืนยันการอนุญาต',
        cancelButtonText: 'ยกเลิก'
      });
      if (!confirm.isConfirmed) return;
      ...
      try {
        ...
        await Swal.fire({
          ...
          text: result.message || 'กำหนดสิทธิ์ผู้ใช้งานเรียบร้อยแล้ว',
          confirmButtonText: 'ตกลง'
        });
