    ...
    async function generateAnnouncementPdf() {
      ...
      showLoading('กำลังนำรายชื่อไปต่อในเท็มเพลและจัดทำ PDF');
      try {
        const file = await serverCall('generateAdminAnnouncementPdf', state.token, {});
        closeLoading();
    ...
    async function renderAnnouncementPdf() {
      setHeader('จัดทำประกาศรายชื่อ', 'ออกรายชื่ออัตโนมัติจากเท็มเพล Google Docs');
      const content = document.getElementById('content');
      ...
      content.innerHTML = `
        <div class="announcement-hero">
          <div>
            ...
            <div class="page-subtitle">ระบบใช้เท็มเพลมาตรฐานของมหาวิทยาลัย และต่อรายชื่อผู้ผ่านการคัดเลือกทั้งหมดอัตโนมัติ · เรียงรหัสคณะตัวที่ 3-4 แล้วปีเข้าศึกษาตัวที่ 1-2</div>
          </div>
          <button class="btn btn-danger" onclick="generateAnnouncementPdf()" ${rows.length ? '' : 'disabled'}>
            <i class="fa-solid fa-file-pdf me-2"></i>ออกรายชื่อ PDF จากเท็มเพล
          </button>
        </div>
        ...
        <div class="panel announcement-settings-panel mb-4">
          <div class="panel-header">
            <div>
              <h2 class="panel-title">เท็มเพลเอกสารที่ระบบใช้</h2>
              <div class="small text-muted mt-1">ระบบจะคัดลอกเท็มเพลนี้อัตโนมัติ แล้วต่อท้ายตารางรายชื่อโดยไม่แก้ไขไฟล์ต้นฉบับ</div>
            </div>
            <a class="btn btn-outline-primary btn-sm" href="https://docs.google.com/document/d/1bslbS9cB8h2xYpa8o42_PtSHyCkQVHI2fD0GTaz0_BU/edit" target="_blank" rel="noopener noreferrer">
              <i class="fa-regular fa-file-word me-1"></i>เปิดเท็มเพล
            </a>
          </div>
          <div class="p-3 pt-0">
            <div class="alert alert-light border mb-0">
              <i class="fa-solid fa-wand-magic-sparkles me-2 text-primary"></i>
              เมื่อกดออกรายชื่อ ระบบจะดึงเฉพาะรายชื่อที่หน่วยงานยืนยัน <strong>พิจารณารับ</strong> จากทุกหน่วยงาน แล้วต่อเป็นตาราง
              <strong>ลำดับที่ | รหัสนักศึกษา | ชื่อ-สกุล | คณะ</strong> ในเท็มเพลโดยอัตโนมัติ
            </div>
          </div>
        </div>
