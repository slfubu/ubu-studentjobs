      const BUILD_VERSION = '2026.09.04-FAST-V3-STAFF';
    ...
    async function loadDepartmentPhotos(rows) {
      ...
      for (let index = 0; index < ids.length; index += 6) {
        chunks.push(ids.slice(index, index + 6));
      }
      ...
      // จำกัด concurrency = 2 เพื่อให้รูปมาเร็วขึ้น แต่ไม่ยิง Apps Script พร้อมกันมากเกินไป
      for (let i = 0; i < chunks.length; i += 2) {
        await Promise.all(chunks.slice(i, i + 2).map(loadChunk));
      }
    }
